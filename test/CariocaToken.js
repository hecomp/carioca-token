...
contract("CariocaToken", function (accounts) {
  ...
  it("initializes the contract with the corect values", function() {
    return CariocaToken.deployed().then(function(instante) {
      tokenInstance = instante;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, "CariocaToken", "has the correct name");
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, "CARI", "has the correct symbol");
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, "Carioca Token v1.0", "has the correct standard");
    })
  })
  ...
  it("transfer token onwership", function() {
    return CariocaToken.deployed().then(function(instance) {
      tokenInstance = instance;
      // Tes `require` statement first by transferring something larger than the sender's balance
  ...
  it("handles delegated token transder",  function() {
    return CariocaToken.deployed().then(function(instance) {
      tokenInstance = instance;
      ...
      // Try transforring soemthing larger than the approved amount
      return tokenInstance.transferFrom(fromAccount, toAccount, 20, { from: spendingAccount })
    }).then(assert.fail).catch(function(error) {
      // console.log(error.message)
      assert(error.message.indexOf('revert') >= 0, 'cannot transfer value larger then approve amount');
